...
// (5) Program
	printk("[AURA_SYNC] Program FW from 0x0000 to 0x3F80 =====\n");
...
static ssize_t red_pwm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[AURA_SYNC] client->addr : 0x%x,  reg_val : 0x%x.\n", client->addr, reg_val);
...
static ssize_t apply_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&g_pdata->ene_mutex);
	if (val > 0){
		printk("[AURA_SYNC] Send apply cmd.\n");
...
static ssize_t vdd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val>0) {
		...
		if ( gpio_is_valid(platform_data->power_gpio) ) {
			...
			if(tmp){
				printk("[AURA_SYNC] already enable.\n");
			}else {
		...
		if ( gpio_is_valid(platform_data->power_gpio) ) {
			...
			tmp = regulator_is_enabled(platform_data->regulator_vdd);
			if(!tmp) {
				printk("[AURA_SYNC] already disable.\n");
			}else {
...
static enum led_brightness aura_sync_get(struct led_classdev *led_cdev)
{
	...
	//printk("[AURA_SYNC] pdata->int_gpio : %d\n", pdata->int_gpio);
	//printk("[AURA_SYNC] pdata->power_gpio : %d\n", pdata->power_gpio);
...
static int ene_8k41_parse_dt(struct device *dev, struct ene_8k41_platform_data *pdata)
{
	...
	printk("[AURA_SYNC] regulator_current : %d\n", pdata->regulator_current);
...
pinctrl:
	...
	if (pdata->dongle) {
		printk("[AURA_SYNC] Get inbox_aura_en \n");
		...
		printk("[AURA_SYNC] Get logo_5p0_en \n");
...
static int ene_8k41_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//printk("[AURA_SYNC] check data[0]: 0x%x\n", data[0]);
...
// Set ENE 8K41 power
	if(gpio_is_valid(platform_data->power_gpio) && platform_data->dongle) {
		...
		if (err)
			printk("[AURA_SYNC] 8k41_power_gpio gpio_request, err %d\n", err);

		printk("[AURA_SYNC] ENE 8k41 power on.\n");
		err = gpio_direction_output(platform_data->power_gpio, 1);
		if (err)
			printk("[AURA_SYNC] power_gpio output high, err %d\n", err);
	}
...
/*
	if(gpio_is_valid(platform_data->int_gpio)) {
		...
		if (err)
			printk("[AURA_SYNC] 8k41_int_gpio gpio_request, err %d\n", err);

		err = gpio_direction_input(platform_data->int_gpio);
		if (err)
			printk("[AURA_SYNC] gpio_direction_intput, err %d\n", err);
	}
*/
	...
	if (!platform_data->dongle) {
		...
		if (err)
			printk("[AURA_SYNC] logo_5v_en gpio_request, err %d\n", err);

		printk("[AURA_SYNC] logo_5v_en default off.\n");
		err = gpio_direction_output(platform_data->logo_5v_en, 0);
		if (err)
			printk("[AURA_SYNC] logo_5v_en output high, err %d\n", err);

	// Set PM845 L22 to ENE 8K41 VDD

		printk("[AURA_SYNC] set VDD %d uA\n", platform_data->regulator_current);
...
// unregister
	...
	// power off
		...
		err = gpio_direction_output(platform_data->power_gpio, 0);
		if (err)
			printk("[AURA_SYNC] power_gpio output low, err %d\n", err);
...
int ene_8k41_suspend(struct device *dev)
{
	...
	printk("[AURA_SYNC] ene_8k41_suspend : current_mode : 0x%x\n", g_pdata->current_mode);

	if (suspend_vdd_on){
		printk("[AURA_SYNC] Keep VDD on when suspend!!!\n");
		return err;
	}

	if(!g_pdata->current_mode){
		//printk("[AURA_SYNC] Disable VDD.\n");
		val = regulator_is_enabled(g_pdata->regulator_vdd);
		if(!val){
			printk("[AURA_SYNC] already Disable!!\n");
		}else{
...
int ene_8k41_resume(struct device *dev)
{
	...
	printk("[AURA_SYNC] ene_8k41_resume : current_mode : 0x%x\n", g_pdata->current_mode);
