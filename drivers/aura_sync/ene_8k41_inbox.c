...
static bool none_register = false;
...
static ssize_t vdd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(gDongleType == 3) {
		printk("[AURA_DT] not support in DT\n");
		return count;
	}
...
static ssize_t ic_status_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	//printk("[AURA_INBOX] Current FW version 0x%x, tmp 0x%x\n", platform_data->fw_version, tmp);
...
// Check FW
	...
	if (platform_data->fw_version == 0xFF){
		printk("[AURA_INBOX] I2C error, may not Jedi Inbox dongle!!!\n");
		goto exit_check_functionality_failed;
	}
...
static int ene_8k41_remove(struct i2c_client *client)
{
	...
	//	kfree(platform_data);
		...
		printk("[AURA_INBOX] none_register is %d.\n", none_register);
...
int ene_8k41_suspend(struct device *dev)
{
	...
	if (gDongleType == 1){
		printk("[AURA_INBOX] ene_8k41_suspend : current_mode : 0x%x\n", g_pdata->current_mode);
...
int ene_8k41_resume(struct device *dev)
{
	...
	if (gDongleType == 1){
		printk("[AURA_INBOX] ene_8k41_resume : current_mode : 0x%x\n", g_pdata->current_mode);
		...
		printk("[AURA_DT] ene_8k41_resume : current_mode : 0x%x\n", g_pdata->current_mode);
	}
