...
static uint16_t g_slave_id = 0x0072;
	...
	if(g_actuator_power_state)
	{
		...
		switch(g_data_type)
		{
			case CAMERA_SENSOR_I2C_TYPE_BYTE:
				rc = actuator_read_byte(actuator_ctrl,g_reg_addr,&reg_val);
				break;
			case CAMERA_SENSOR_I2C_TYPE_WORD:
				rc = actuator_read_word(actuator_ctrl,g_reg_addr,&reg_val);
				break;
			case CAMERA_SENSOR_I2C_TYPE_DWORD:
				rc = actuator_read_dword(actuator_ctrl,g_reg_addr,&reg_val);
				break;
			default:
				rc = actuator_read_dword(actuator_ctrl,g_reg_addr,&reg_val);
		}
...
static ssize_t actuator_i2c_debug_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	if(g_operation == 1)
	{
		...
		{
			case CAMERA_SENSOR_I2C_TYPE_BYTE:
				rc = actuator_write_byte(actuator_ctrl,g_reg_addr,g_reg_val);
				break;
			case CAMERA_SENSOR_I2C_TYPE_WORD:
				if(g_reg_addr == 0xFFFE)
				{
					...
					rc = actuator_write_word(actuator_ctrl,g_reg_addr,vcm_register_value);
					//pr_info("Dac value 0x%x register value 0x%04x\n",g_reg_val,vcm_register_value);
				}
				else
					rc = actuator_write_word(actuator_ctrl,g_reg_addr,g_reg_val);
				break;
			case CAMERA_SENSOR_I2C_TYPE_DWORD:
				rc = actuator_write_dword(actuator_ctrl,g_reg_addr,g_reg_val);
				break;
			default:
				rc = actuator_write_byte(actuator_ctrl,g_reg_addr,g_reg_val);
		}
		...
		else
		{
			...
			g_atd_status = 1;
			//for update /proc/driver/vcm value
			if(g_data_type == CAMERA_SENSOR_I2C_TYPE_WORD && g_reg_addr == 0xA0)
			{
				set_vcm_lens_pos_from_ois_writing(g_reg_val);
			}
		}
	}
