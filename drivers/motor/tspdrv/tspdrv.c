...
**
** Portions Copyright (c) 2008-2018 Immersion Corporation. All Rights Reserved.
**
...
#define DEVICENAME "tspdrv"
static struct class *tspdrv_class; 
static struct cdev *tspdrv_cdev; 

//end

/* This driver outputs force values through alsa (not 'write')
   and uses tspdrv for wake lock mechanism. The player service
   does not call write. */
#ifndef ALSA_PCM_OUTPUT
#define ALSA_PCM_OUTPUT
#endif
...
#ifdef IMPLEMENT_AS_CHAR_DRIVER
	dev = MKDEV(g_nMajor, 0);
	nRet=alloc_chrdev_region(&dev, 0, 1, DEVICENAME);
	if (nRet < 0) {
		DbgOutErr(("cannot allocate major number %d\n", g_nMajor));
		return nRet;
	}
	tspdrv_class = class_create(THIS_MODULE, DEVICENAME);
	g_nMajor = MAJOR(dev);
	DbgOutInfo(("allocated Major Number: %d\n", g_nMajor));

	tspdrv_cdev = cdev_alloc();
	cdev_init(tspdrv_cdev, &fops);
	tspdrv_cdev->owner = THIS_MODULE;
	tspdrv_cdev->ops = &fops;

	if (device_create(tspdrv_class, NULL, dev, NULL, "tspdrv") == NULL)
		DbgOutErr(("Device creation failed\n"));

	if (cdev_add(tspdrv_cdev, dev, 1) < 0) {
		DbgOutErr(("tspdrv_driver: cdev_add failed\n"));
...
#ifdef ALSA_PCM_OUTPUT
            /* since timer is not used, open wake lock here */
