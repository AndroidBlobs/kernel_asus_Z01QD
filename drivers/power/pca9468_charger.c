...
#endif
#include <linux/uaccess.h>			// NTC compenstation
...
#define PCA9468_ICHG_DONE_DFT	500000	// 500mA
...
/* CC mode 1,2 IIN_CFG */
#define PCA9468_CC1_IIN_CFG_4A		1900000	// 1900000uA, panel On
#define PCA9468_CC1_IIN_CFG_3A		1400000	// 1400000uA, panel Off
...
#define JEITA_STAGE3_IIN			900000	// 900000uA
...
/* NTC Node */
#define NTC_NODE_PATH		"/sys/devices/platform/soc/c440000.qcom,spmi/spmi-0/spmi0-02/c440000.qcom,spmi:qcom,pmi8998@2:rradc@4500/iio:device0/in_temp_skin_temp_input"
/* NTC threshold */
 ...
 * @ta_max_cur: TA maximum current for the direct charing, uA
			...
			if (pca9468->jeita_stage == JEITA_STAGE2) {
				...
				if (pca9468->ntc_stage == NTC_STAGE1) {
					...
					if (pca9468->ta_cur > NTC_STAGE2_IIN) {
						...
						pr_info("[PCA] %s: NTC stage2, ta_cur:%d(mA)\n", __func__, pca9468->ta_cur);
					}
				}
			}
...
static int pca9468_preset_dcmode(struct pca9468_charger *pca9468)
{
	...
	if (pca9468->jeita_stage == JEITA_STAGE3) {
		...
		if (g_fv_setting == 0)
			...
			pca9468->pdata->v_float = 3487500 + 7500*g_fv_setting + 23000;
	} else {
	...
	case TIMER_PDMSG_SEND:
		...
		if (pca9468->charging_state == DC_STATE_PRESET_DC) {
			...
			if (g_fv_setting == 0)
				...
				ret = pca9468_set_switching_charger(true, SWCHG_ICL_MAX, SWCHG_ICL_MAX, 3487500 + 7500*g_fv_setting + 23000);
			if (ret < 0)
				goto error;
			/* Notifier PMIC PCA JEITA STOP */
