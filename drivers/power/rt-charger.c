...
int rt_chg_ready_snk_apdo(struct notifier_block *nb) {
	...
	if (info->pd_apdo_connected == false) {
			...
			rt_charger_set_usb_property_notifier(POWER_SUPPLY_PROP_PD_ALLOWED, 0);
			/*val.intval = 0;
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case	PD_CONNECT_PE_READY_SNK_PD30:
			pr_info("%s tcpc_pd_state = PD_CONNECT_PE_READY_SNK_PD30\n", __func__);
			break;
		case	PD_CONNECT_PE_READY_SNK:
			pr_info("%s tcpc_pd_state = PD_CONNECT_PE_READY_SNK,\n", __func__);
			break;
...
static int isKeep5v(int vid, int pid){
	 ...
	 */
	if ((vid == 10232 && pid == 3005) || (vid == 7531 && pid == 2) || (vid == 7531 && pid == 3))
		return 1;
	else
		return 0;
}
