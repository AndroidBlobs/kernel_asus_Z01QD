...
void (*report_station_amp)(int); //Austin +++
EXPORT_SYMBOL(report_station_amp);
...
extern void asus_dp_change_state(bool mode, int type); //ASUS_Display+++
extern void ec_get_hal_sensor_status(int); //ASUS_BSP Deeo : For JEDI station EC driver
extern void ec_hid_reset_vote(void);
...
/*
		if (gDongleType == 2) {
			log("[EC_HID] trigger USB turn on HOST\n");
			asus_station_enable_host(1);
		}
*/
		if (report_station_amp)
			report_station_amp(1); //Austin+++
		ec_hid_reset_vote();
        ec_get_hal_sensor_status(0);
        asus_dp_change_state(true, 0);//ASUS_Display+++, notify dp
	  } else {
		hallsensor_report_lid(HALLSENSOR_REPORT_LID_CLOSE);
		if (report_station_amp)
			report_station_amp(0); //Austin+++
        asus_dp_change_state(false, 0);//ASUS_Display+++, notify dp
        ec_get_hal_sensor_status(1);
	  }
