...
static int cm36656_regulator_init(void){
    ...
    log("ldoa19 regulator setting init");
    return ret;
}
...
static int cm36656_regulator_enable(void){
    ...
    for(idx=0; idx<10; idx++){
        if(regulator_is_enabled(reg) > 0){
            dbg("ldoa19 regulator is enabled(idx=%d)", idx);
            break;
        }
    }
    if(idx >= 10){
        err("ldoa19 regulator is enabled fail(tryty count >= %d)", idx);
        return -1;
    }
    
    log("Update ldoa19 to NPM_mode");
    return ret;
}
