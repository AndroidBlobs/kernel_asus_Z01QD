...
static int rpm_stats_suspend(struct device *dev)
{
	...
	for (i = 0; i < 2; i++) {
		...
		printk(KERN_INFO "[RPM] Suspend: Voting status: Mode: %s, Count: %d, Mask:0x%llx, Status(before wfi): 0x%llx, Status(after wfi): 0x%0llx\n",
			stat_type, data.count, data.accumulated, data.last_entered_at, data.last_exited_at);
	}
...
static int rpm_stats_resume(struct device *dev)
{
	...
	for (i = 0; i < 2; i++) {
		...
		printk(KERN_INFO "[RPM] Resume: Voting status: Mode: %s, Count: %d, Mask:0x%llx, Status(before wfi): 0x%llx, Status(after wfi): 0x%0llx\n",
			stat_type, data.count, data.accumulated, data.last_entered_at, data.last_exited_at);
	}
