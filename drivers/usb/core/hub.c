...
{
	dev_info(&udev->dev, "[USB] New USB device found, idVendor=%04x, idProduct=%04x, bcdDevice=%04x\n",
	...
	if (le16_to_cpu(udev->descriptor.idVendor)==0x05E3 && le16_to_cpu(udev->descriptor.idProduct)==0x0F01) {
		dev_info(&udev->dev, "[USB_DT] Billboard enable autosuspend\n");
