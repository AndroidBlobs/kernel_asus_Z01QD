	...
	struct dpm_pdo_info_t *source, struct dpm_pdo_info_t *sink)
{
	...
	printk("[Bottom_PD] dpm_calc_src_cap_power_uw, mv=%d, ma=%d, uw=%d\n", source->vmax,  source->ma, uw);
	return uw;
}
	...
	struct dpm_pdo_info_t *sink, struct dpm_pdo_info_t *source)
{
	...
	if (uw == 14850000 && source->vmax == 9000)
		uw =15030000;

	if (select_info->max_uw < uw || (select_info->max_uw == uw && select_info->cur_mv  < source->vmax) || (select_info->max_uw == uw && select_info->cur_mv==9000)){
		if (uw == 15030000)
			select_info->max_uw = 14850000;
		else
			select_info->max_uw = uw;
		select_info->cur_mv = source->vmax;
		printk("[Bottom_PD] pdo_select, uw=%d, mv=%d\n", select_info->max_uw , select_info->cur_mv);
		return true;
	}
