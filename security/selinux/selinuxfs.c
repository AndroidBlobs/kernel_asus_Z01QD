...
static ssize_t ec_write_asus(struct file *file, const char __user *buf,
					size_t count, loff_t *ppos)
{
	char *page = NULL;
	ssize_t length;
	...
	if (new_value != security_get_ec()) {
		security_set_ec(new_value);
	}
	length = count;
out:
	free_page((unsigned long) page);
...
static ssize_t ec_read_asus(struct file *filp, char __user *buf,
					size_t count, loff_t *ppos)
{
	char tmpbuf[TMPBUFLEN];
	ssize_t length;
...
static const struct file_operations sel_aps_ec = {
	.read		= ec_read_asus,
	.write		= ec_write_asus,
	.llseek		= generic_file_llseek,
};
...
static int sel_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	static struct tree_descr selinux_files[] = {
		...
		[SEL_EC] = {"ec", &sel_aps_ec, S_IRUGO|S_IWUSR},
		/* last one */ {""}
	};
